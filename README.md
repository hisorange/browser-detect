## Browser Detection 4 by _[hisorange](https://hisorange.me)_
...
Easy to use package to identify the visitor's browser details and device type. Magic is **not** involved the results are generated by multiple well tested and developed packages. Supporting every **laravel** version between **4.0 &raquo; 6.0**, also tested on every release **PHP** between **5.6 &raquo; 7.2**.
...
Laravel versions below 5.4 are not actively developed but you can still use the browser detect 3.x for it; You can find those releases under the [versions](https://github.com/hisorange/browser-detect/releases) tab. Please read the readme from the release use choose, after version 3.x the package dropped the buggy workarounds to support the PHP 5.x syntax.
