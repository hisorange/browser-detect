...
class Parser {
	 ...
	 * CLI or browser without user agent header uses the generic agent.
	 *
	 * @return string
	 */
	public function visitorsAgent()
	{
		// Generic values.
		$generic 	= $this->objectConfigGet('generic');

		return $this->app['request']->server('HTTP_USER_AGENT', $generic['agent']);
	}

	/**
	 * Get the reflected user agent's informations.
	 * If null is setted then the class will use the visitor's agent.
	 * If cache interval setted to 0 the func skip to load the cache.
	 *
	 * @since 1.0.0
	 *
	 * @param  string|null $agent User agent HTTP header.
	 * @return \hisorange\BrowserDetect\Result
	 */
	public function detect($agent = null)
	{
		// Use fallback values for the user agent.
		$agent  	= $agent ?: $this->visitorsAgent();

		// Get cache configuration.
		$cache 		= $this->objectConfigGet('cache');

		// Generate cache key for a user-agent string.
		$key 		= $cache['prefix'] .  $this->hashUserAgentString($agent);
		...
		if ($cache['interval']) {
			...
			$cachedResult 	= $this->app['cache']->remember($key, $cache['interval'], function() use ($agent) {
				return $this->parse($agent)->toString();
			});

			// Convert the result back to an object.
			$result 		= $this->getEmptyResult()->importFromString($cachedResult);
		} else {
			$result 		= $this->parse($agent);
		}
	...
	/**
	 * @since 1.0.0 Hash the user-agent string, function created to be easly extendable.
	 *
	 * @param  string $agent
	 * @return string
	 */
	...
	public function parse($agent)
	{
		...
		foreach ($this->objectConfigGet('plugins') as $plugin => $config) {
			// Create the plugin instance.
			$plugin 	= new $plugin;

			// Import the object config.
			$plugin->objectConfigImport($config);

			// Merge the result with the schema.
			$result 	= array_merge($result, $plugin->filter($plugin->parse($agent)));
		}
		...
		$result['osFamily']			= $result['osFamily'] ?: $generic['operatingsystem'];

		// Fixing empty browser family with generic value.
		$result['browserFamily']	= $result['browserFamily'] ?: $generic['browser'];
